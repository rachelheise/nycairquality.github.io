...
#overall
builder_test = ggplot_build(covid_aqi_overall_ggplot)
...
sp.gam.covid = gam(total_observed_value ~ s(ndate, bs = "ad"), data = AQI_covid_master, method = "REML")
sp.pred.covid = predict.gam(sp.gam.covid, se.fit = TRUE)
#create GAM lines for AQI overlay
sp.gam.aqi = gam(AQI ~ s(ndate, bs = "ad"), data = AQI_covid_master, method = "REML")
sp.pred.aqi = predict.gam(sp.gam.aqi, se.fit = TRUE)
...
renderPlotly({
  ...
  plot_ly() %>% 
    ...
    add_lines(
      x = ~date,
      y = ~sp.pred.covid$fit,
      name = "COVID Count GAM",
      type = 'scatter',
      mode = "lines",
      color = ~total_observation_type,
      colors = "Blues",
      opacity = 1) %>% #covid gam
    add_lines(
      x = ~date,
