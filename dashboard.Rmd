...
renderLeaflet({
  AQI_covid_master %>% 
    ...
    mutate(popup = str_c("<strong>", borough, "</strong>", "<br/>", "Date: ", date, "<br/>", "Air Quality Category: ", AQI_category, "<br/>", "Air Quality Index: ", round(AQI, 2), "<br/>", "Carbon Monoxide Level: ", round(IAQI_co, 2)) %>% 
    map(htmltools::HTML)) %>%
    leaflet() %>%
    addTiles() %>%
