...
AQI_2019_and_2020 = #merge the 2019 and 2020
...
nyc_daily_borough_testing = # prep covid data, initial cleaning
  read_csv(
...
```{r, echo = F}
AQI_by_borough = AQI_covid_master %>% #create the rolling averages for the AQI, per borough
...
```{r, echo = F}
AQI_covid_master = AQI_covid_master %>% #add the per borough to the masterdataset w/ covid
  left_join(total_AQI, by = "date") %>% 
  left_join(AQI_by_borough, by = c("date", "borough"))
```

```{r vline function, echo = F}
vline <- function(x = 0, color = "red") { #brief function to flexibly create vertical line shapes on plotly
  list(
    type = "line", 
    y0 = 0, 
    y1 = 0.9, 
    ...
    x0 = x, 
    x1 = x, 
    line = list(color = color, opacity = 0.75, dash = "dash", width = 1)
  )
}
```
...
AQI_dif = right_join(AQI_total_2019, AQI_total_2020, by = c("month", "day")) %>% 
  ...
  filter(year.x == 2019,
         year.y == 2020) %>% 
  mutate(percent_dif_AQI = ((avg_AQI - avg_AQI_2019)*100/avg_AQI_2019)) %>% 
  mutate(year = year.y) %>% 
  select(year, month, day, percent_dif_AQI) %>% 
  unite("date", year:day, sep = "-") %>% 
  mutate(date = as.Date(date)) %>% 
  arrange(date) %>% 
  mutate(aqi_dif_7day_avg = zoo::rollmean(percent_dif_AQI, k = 7, fill = NA))
...
IAQI_co_borough_dif =
  ...
  mutate(percent_dif_IAQI_co = ((IAQI_co_2020 - IAQI_co_2019)*100/IAQI_co_2019)) %>%  
  drop_na(percent_dif_IAQI_co) %>% 
  select("date", "borough", "percent_dif_IAQI_co") %>% 
  mutate(
    date = str_c("2020-", date),
    date = as.Date(date)
  ) %>%
  filter(percent_dif_IAQI_co != Inf) %>% 
  arrange(date) %>% 
  mutate(IAQI_co_dif_7day_avg = zoo::rollmean(percent_dif_IAQI_co, k = 7, fill = NA)) %>% 
...
### AQI and COVID Cases Per Borough
...
renderPlotly({
  ...
  plot_ly(
      x = ~date,
      y = ~aqi_dif_7day_avg,
      name = "AQI % Diff. from 2019 (7 Day Rolling Avg.)",
      type = "scatter",
      mode = "lines",
      opacity = 0.8,
      text = ~text_label_1,
    ...
    layout(
      shapes = list(vline("2020-03-22"), vline("2020-08-08")),
      xaxis = list(title = "Date"),
      yaxis = list(side = "left", title = "AQI Percent Difference from 2019 (7 Day Rolling Avg.)", showgrid = FALSE, zeroline = FALSE),
...
renderPlotly({
  AQI_covid_master %>%
    ...
    layout(
      shapes = list(vline("2020-03-22"), vline("2020-08-08")),
      xaxis = list(title = "Date"),
      yaxis = list(side = "left", title = "Carbon Monoxide Air Quality Index (7 Day Average)", showgrid = FALSE, zeroline = FALSE),
      yaxis2 = list(side = "right", overlaying = "y", title = "New Case Count (Persons)", showgrid = FALSE, zeroline = FALSE)
      )
  })
```
